...
const ViewOfContent = ({ content, test, courseId }) => {
  ...
  const handleChange = (event, value) => {
    const currentProgress = currentUser.courses.find(
    ...
    setProgress((prev) => ({
      ...
      courseProgressPercent: Number(
        ((currentProgress / prev.courseLength) * 100).toFixed(2)
      ),
    }));
    ...
    updateProgress(currentUser.id, {
      ...
      courseProgress: currentProgress > value ? currentProgress : value,
      courseProgressPercent: Number(
        ((currentProgress / progress.courseLength) * 100).toFixed(2)
      ),
    })
      .then(() => {
        setPage(value);
    ...
    const fetchData = async () => {
      try {
        ...
        content?.forEach((contentItem) => {
          const matchingTest = test.find(
            (testItem) => contentItem.title === testItem.title
          );
          if (matchingTest) {
  ...
  const handleTestSubmit = (event) => {
    ...
    const body = {
      ...
      testsResults: {
        ...
        responses: Object.entries(selectedOptions).map(
          ([question, response]) => ({
            question,
            response,
          })
    ...
    updateTest(currentUser.id, body)
      .then((data) => {
        console.log(data);
          ...
          {contentArray[page - 1].questions ? (
            existingTest(contentArray[page - 1]) ? (
              ...
              >
                ...
                <Typography variant="body">
                  Tu puntuación fue de: {findScore(contentArray[page - 1])}
                </Typography>
            ...
            <Card
              sx={{
                ...
                justifyContent: "center",
                alignItems: "center",
                marginY: { xs: 2, md: 2, lg: 4 },
              }}
            >
              <Typography variant='h5'>{contentArray[page - 1].title}</Typography>
              <Box
                sx={{
                  marginY: 2,
              ...
              >
                <Box
                  sx={{
                    display: "flex",
                    justifyContent: "center",
                    maxWidth: { xs: "100%", md: "100%", lg: "50%" },
                  }}
                >
                 {   
                 contentArray[page - 1].description.toString() !== 'undefined' ?
                  <TextFormatToShowInCourseContent
                    htmlContent={contentArray[page - 1].description}
                  />
                : null
                }
                </Box>
                {
                  (contentArray[page - 1].image.toString() !== "undefined" ?
                  <Box sx={{ display: "flex", justifyContent: "center" }}>
                  <img
                    src={contentArray[page - 1].image}
                    alt={contentArray[page - 1].title}
                    style={{
                      maxWidth: "100%",
                      maxHeight: "100%",
                      objectFit: "contain",
